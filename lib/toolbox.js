const { sum, isInteger } = require('mathjs');
...
const nthLexicographicPermutation = (n, elements) => {
  ...
  if (n > maxPermutationCount) {
    throw new Error(`최대 ${maxPermutationCount}가지 순열이 존재합니다`);
  }
...
const numberOfPartitions = (n, parts) => {
  ...
  let orderedParts = null;

  if (parts === undefined) {
    orderedParts = Array(n).fill(null).map((_, index) => (n - index));
  }
  ...
  const recursion = (num, sortedParts) => {
    if (num === 0) {
      return 1;
    }
 ...
 * @param {number} n
 */
const numberOfIntegerPartitions = (() => {
  const storedResult = {
    0: 1n,
    1: 1n,
  };
